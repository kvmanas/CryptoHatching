  ...
  <div class="unitmodel col-lg-4 col-md-6 ml-auto mr-auto" v-loading="isLoading">
    <div class="card card-coin card-plain">
      <div class="card-header">
        ...
        <p class="card-category unitpower">{{UserPiggs}}/{{MaxUnit}}</p>
      </div>
      <div class="card-body">
        <div class="container buyunit">
          <h4 class="unitpower">
            <p>Attack: {{AttackRate}}</p>
            <p>Defence: {{DefenceRate}}</p>
...
export default {
  ...
  computed: {
    ...
    AttackRate() {
      try {
        if (this.UserData[this.UnitData[2]].Power == 1) {
          return (
            parseInt(this.UnitData[6]) + parseInt(this.UnitData[8][1].rate)
          );
        } else if (this.UserData[this.UnitData[2]].Power == 2) {
          return (
            parseInt(this.UnitData[6]) +
            parseInt(this.UnitData[8][1].rate) +
            parseInt(this.UnitData[8][2].rate)
          );
          ...
          return (
            ...
            parseInt(this.UnitData[8][1].rate) +
            parseInt(this.UnitData[8][2].rate) +
            parseInt(this.UnitData[8][3].rate)
          );
        } else {
    ...
    DefenceRate() {
      try {
        if (this.UserData[this.UnitData[2]].Power == 1) {
          return (
            parseInt(this.UnitData[7]) + parseInt(this.UnitData[8][1].rate)
          );
        } else if (this.UserData[this.UnitData[2]].Power == 2) {
          return (
            parseInt(this.UnitData[7]) +
            parseInt(this.UnitData[8][1].rate) +
            parseInt(this.UnitData[8][2].rate)
          );
        } else if (this.UserData[this.UnitData[2]].Power == 3) {
          ...
          return parseInt(this.UnitData[7]);
        }
      } catch (e) {
        return parseInt(this.UnitData[7]);
      }
    ...
    PowerRate() {
      try {
        return this.UnitData[8][this.UserData[this.UnitData[2]].Power + 1].rate;
      } catch (e) {
        return this.UnitData[8][1].rate;
      }
    },
    isPowerMax() {
      try {
  ...
  methods: {
    MaxBtn() {
      ...
      if (Qty > this.MaxUnit - this.UserPiggs) {
        this.BuyQty = this.MaxUnit - this.UserPiggs;
      } else {
        this.BuyQty = Qty;
      }
    },
    ...
    async BuyPower() {
      if (this.EggBalance < this.PowerPrice) {
        ...
        this.isLoading = true;
        const Gameclient = new GameClient(localStorage.PrvKey);
        var formdata = [2, this.UnitData[2], this.PowerPrice];
    ...
    async BuyMax() {
      if (this.EggBalance < this.MaxPrice) {
        ...
        this.isLoading = true;
        const Gameclient = new GameClient(localStorage.PrvKey);
        var formdata = [2, this.UnitData[2], this.MaxPrice];
